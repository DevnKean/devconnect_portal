...
class DataAcquisition extends AbstractFormatter implements FormatterInterface
{
    ...
    public function format(LogEntryInterface $log)
    {
        ...
        if ($log->isCreate()) {
            $result['message'] = '<dl><dt>The acquisition has been created.</dt>%s</dl>';
            ...
            foreach ($data as $field => $value) {
                ...
                $text .= sprintf('<dd>The acquisition has been changed: %s</dd>', $subText);
            }
