...
class DataAcquisitionProvider extends AbstractFormatter implements FormatterInterface
{
    ...
    public function format(LogEntryInterface $log)
    {
        ...
        if ($log->isCreate()) {
            $result['message'] = '<dl><dt>The acquisition provider has been created.</dt>%s</dl>';
            ...
            foreach ($data as $field => $value) {
                ...
                $text .= sprintf('<dd>The acquisition provider has been changed: %s</dd>', $subText);
            }
