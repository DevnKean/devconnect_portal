...
class Supplier extends AbstractFormatter implements FormatterInterface
{
    ...
    public function format(LogEntryInterface $log)
    {
        ...
        if ($log->isCreate()) {
            $result['message'] = sprintf('One <strong><span class="font-green-jungle">"%s"</span></strong> was created.', 'certification');
            ...
            $result['message'] = '<dl><dt>The <strong><span class="font-yellow-gold">"%s"</span></strong> profile section has been updated.</dt>%s</dl>';
            $data = $log->getData();
