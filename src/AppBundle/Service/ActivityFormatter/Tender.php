...
class Tender extends AbstractFormatter implements FormatterInterface
{
    ...
    public function format(LogEntryInterface $log)
    {
        ...
        if ($log->isCreate()) {
            $result['message'] = '<dl><dt>The tender has been created.</dt>%s</dl>';
            ...
            foreach ($data as $field => $value) {
                ...
                $text .= sprintf('<dd>The tender has been changed: %s</dd>', $subText);
            }
